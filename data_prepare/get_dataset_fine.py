...
sigmas=[0.8, 0.6, 0.4, 0.2, 1, 2, 4, 6, 8]
...
if __name__ == "__main__":
    ...
    for sigma in sigmas:
        ...
        os.makedirs('../dataset_fine/dataset_'+str(sigma).replace('.',''), exist_ok=True)
        os.makedirs('../dataset_fine/dataset_'+str(sigma).replace('.','')+'/data/'+ mode +'/', exist_ok=True)
        os.makedirs('../dataset_fine/dataset_'+str(sigma).replace('.','')+'/gt/'+ mode +'/', exist_ok=True)
    for files in tqdm(img_list, desc="Processing images"):
        srcImg = cv.imread('fine_picture/' + files)
        srcImg = cv.resize(srcImg, (400, 400))
        for sigma in sigmas: 
            k1 = random.uniform(k1_down, k1_top) * sigma
            k2 = random.uniform(k2_down, k2_top) * sigma
            k3 = random.uniform(k3_down, k3_top) * sigma
            k4 = random.uniform(k4_down, k4_top) * sigma

            dstImg, cutImg, source_Img = create_fish(srcImg, k1, k2, k3, k4)

            cv.imwrite('../dataset_fine/dataset_'+str(sigma).replace('.','')+'/data/'+ mode +'/' + files + '.jpg', cutImg)
            cv.imwrite('../dataset_fine/dataset_'+str(sigma).replace('.','')+'/gt/'+ mode +'/' + files + '.jpg', source_Img)
        num = num + 1
