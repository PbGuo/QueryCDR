...
if __name__ == "__main__":
    for root, dirs, img_list in os.walk(path):
        print(img_list)
    os.makedirs('../dataset_pre/data/'+ mode, exist_ok=True)
    os.makedirs('../dataset_pre/gt/'+ mode, exist_ok=True)
    ...
    for files in img_list:
        ...
        print(files)
        srcImg = cv.imread('pre_picture/' + files)
        srcImg = cv.resize(srcImg, (400, 400))
        dstImg, cutImg, source_Img = create_fish(srcImg, k1, k2, k3, k4)

        cv.imwrite('../dataset_pre/data/'+ mode +'/' + str(num) + '.jpg', cutImg)
        cv.imwrite('../dataset_pre/gt/'+ mode +'/' + str(num) + '.jpg', source_Img)
        num = num + 1
