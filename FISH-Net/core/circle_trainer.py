...
class Trainer():
    ...
    def load(self):
        ...
        if self.config['loadfrom'] == True:
            model_path = self.config['loadfrom']
    ...
    def _train_epoch(self):
        ...
        for images, gt, _, _ in self.train_loader:
            ...
            if self.epoch < 10:
                ...
                rand_count= self.epoch%3

                for _ in range(rand_count):
                    dim = random.randint(1, 128)  # 随机半径
                    ring_width = random.randint(5, 10)  # 随机圆环宽度

                    for i in range(b):
                        center = (h // 2, w // 2)  # 中心点坐标
                        y, x = torch.meshgrid(torch.arange(h) - center[0], torch.arange(w) - center[1])
                        # 创建圆环掩码
                        mask_ring = ((x ** 2 + y ** 2) <= (dim) ** 2) & ((x ** 2 + y ** 2) >= (dim - ring_width) ** 2)
                        mask[i, :, mask_ring] = 0
            ...
            feats, pred_img = self.netG(masked_images)
            comp_img = pred_img
