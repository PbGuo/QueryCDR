...
class Dataset(torch.utils.data.Dataset):
  def __init__(self, data_args, debug=False, split='train', level=None):
    ...
    self.data = [os.path.join("../", i)
      for i in np.genfromtxt(os.path.join(data_args['flist_root'], data_args['name'], split+'.flist'), dtype=np.str_, encoding='utf-8')]
    self.gt = [os.path.join("../", i)
      for i in np.genfromtxt(os.path.join(data_args['flist_root'], data_args['name'], split+'_gt.flist'), dtype=np.str_, encoding='utf-8')]
    self.data.sort()
    self.gt.sort()
