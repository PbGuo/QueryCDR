...
from core.controll_trainer import Trainer
...
parser.add_argument('--loadnum', default=None, type=str)
parser.add_argument('--finetune', action='store_true')
parser.add_argument('--loadstrict', action='store_false')
...
def main_worker(gpu, ngpus_per_node, config):
  ...
  config['loadfrom'] = False
  config['loadstrict'] = args.loadstrict
  if not config['loadstrict']:
    print('load strict == FALSE!!!')
  if args.load is not None and args.loadnum is not None:
    config['loadfrom'] = True
    config['true_save_dir'] = config['save_dir']
    config['save_dir'] = args.load
    config['loadnum'] = args.loadnum
  else:
    config['save_dir'] = os.path.join(config['save_dir'], '{}_{}_{}{}'.format(config['model_name'], config['data_loader']['name'], config['data_loader']['mask'], config['data_loader']['w']))
    config['true_save_dir'] = config['save_dir']
  if (not config['distributed']) or config['global_rank'] == 0:
    os.makedirs(config['true_save_dir'], exist_ok=True)
    print('[**] create folder {}'.format(config['true_save_dir']))
