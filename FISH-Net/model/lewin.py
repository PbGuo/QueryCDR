...
class LinearProjectionqkv(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0., bias=True):
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
    ...
    def forward(self, x, y, attn_kv=None):
        B_, N, C = x.shape
        if attn_kv is not None:
            attn_kv = attn_kv.unsqueeze(0).repeat(B_,1,1)
        else:
        ...
        N_kv = attn_kv.size(1)
        q = self.to_q(x).reshape(B_, N, 1, self.heads, C // self.heads).permute(2, 0, 3, 1, 4)
        k= self.to_k(y).reshape(B_, N_kv, 1, self.heads, C // self.heads).permute(2, 0, 3, 1, 4)
        v = self.to_v(y).reshape(B_, N_kv, 1, self.heads, C // self.heads).permute(2, 0, 3, 1, 4)
        q = q[0]
        k, v = k[0], v[0] 
        return q,k,v
...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size,num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        if token_projection =='conv':
            self.q = ConvProjection(dim,num_heads,dim//num_heads,bias=qkv_bias)
            self.kv = ConvProjection(dim,num_heads,dim//num_heads,bias=qkv_bias)
        elif token_projection =='linear':
            self.q = LinearProjectionqkv(dim,num_heads,dim//num_heads,bias=qkv_bias)
        else:
...
class LeWinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, mask=None):
        ...
        y_windows = window_partition(y, self.win_size)  # nW*B, win_size, win_size, C  N*C->C
        y_windows = y_windows.view(-1, self.win_size * self.win_size, C)  # nW*B, win_size*win_size, C

        wmsa_in = x_windows

        # W-MSA/SW-MSA
        attn_windows = self.attn(wmsa_in, y_windows, mask=attn_mask)  # nW*B, win_size*win_size, C
